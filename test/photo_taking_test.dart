...
void main() {
  group('Photo Taking Test', () {
    test('Take a photo using ImagePicker', () async {
      ...
      // Mock the `pickImage` method to return a sample image file
      final sampleImage = XFile('test/assets/sample_image.jpg');

      // When `pickImage` is called with camera source, then answer with a sample image
      when(mockImagePicker.pickImage(source: ImageSource.camera)).thenAnswer((_) async => sampleImage);
